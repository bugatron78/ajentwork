...
func DefaultRegistry() Registry {
	...
	commands := []Doc{
		...
		},
		{
			...
			Purpose: "Use the Jira adapter to pull Jira issues into local aj items, push local items to Jira, link existing local items, sync linked items with conflict detection, and send compact milestone comments back to Jira.",
			Usage:   "aj jira <pull|push|link|sync|comment> ...",
			Options: []OptionDoc{
				...
				{Name: "link <id> <key>", Description: "attach an existing local item to an existing Jira issue"},
			...
			Safety:         []string{"Set Jira credentials through environment variables instead of committing secrets into .aj/config.toml.", "Use `aj jira sync --dry-run` before the first sync or whenever local and remote changes may have diverged.", "Sync may attempt a Jira status transition when local status differs and a mapped transition exists."},
			WorkflowTags:   []string{"jira", "integration"},
			SearchKeywords: []string{"jira", "import", "export", "sync", "comment", "transition"},
		},
		{
			Name:    "init",
	...
	workflows := map[string]WorkflowDoc{
		...
		"jira": {
			...
			Steps: []string{
				...
				"4. Use `aj update <id> --summary ... --next ...` to record progress, `aj jira comment <id> --summary \"ready for review\"` for milestone updates, or `aj jira sync <id> --dry-run` to preview sync direction and status transitions.",
			},
		},
		"blocked": {
			Name:  "blocked",
