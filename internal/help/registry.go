...
func DefaultRegistry() Registry {
	...
	commands := []Doc{
		...
		},
		{
			...
			Purpose: "Use the Jira adapter to pull Jira issues into local aj items, push local items to Jira, link existing local items, and sync linked items with conflict detection.",
			Usage:   "aj jira <pull|push|link|sync> ...",
			Options: []OptionDoc{
				...
				{Name: "sync <id>", Description: "sync a linked local item with Jira; use --dry-run to preview direction"},
			...
			Safety:         []string{"Set Jira credentials through environment variables instead of committing secrets into .aj/config.toml.", "Use `aj jira sync --dry-run` before the first sync or whenever local and remote changes may have diverged."},
	...
	workflows := map[string]WorkflowDoc{
		...
		"jira": {
			...
			Steps: []string{
				...
				"4. Use `aj update <id> --summary ... --next ...` to record progress, `aj jira link <id> ABC-123` to attach an existing local item, or `aj jira sync <id> --dry-run` to preview sync direction.",
			},
		},
		"blocked": {
			Name:  "blocked",
