...
func DefaultRegistry() Registry {
	...
	commands := []Doc{
		...
		},
		{
			...
			Purpose: "Use the Jira adapter to pull Jira issues into local aj items, push local items to Jira, link existing local items, inspect status mappings and transitions, sync linked items with conflict detection, and send compact milestone comments back to Jira.",
			Usage:   "aj jira <pull|push|link|status-map|transitions|sync|comment> ...",
			Options: []OptionDoc{
			...
			Safety:         []string{"Set Jira credentials through environment variables instead of committing secrets into .aj/config.toml.", "Use `aj jira status-map` and `aj jira transitions <id>` before the first sync or whenever local and remote workflow behavior is unclear.", "Sync may attempt a Jira status transition when local status differs and a mapped transition exists."},
			WorkflowTags:   []string{"jira", "integration"},
			SearchKeywords: []string{"jira", "import", "export", "sync", "comment", "transition", "status map", "workflow"},
		},
		{
			Name:    "init",
